...
static void load_config(void)
{
    ...
    int in_section = 0;
    /* 1 行の最大長
     * 配列は 1 行記述のみ対応するため、宣言容量の上限を 1 行に収める必要がある。
     * 最悪ケースは classes = [ が 11 バイト、32 エントリ × 257 バイト（255 文字 +
     * 前後のクォート）が 8224 バイト、区切りの , と後続の空白が 2 バイト × 31 個で
     * 62 バイト、] が 1 バイト、改行が 1 バイトの計 8299 バイトとなる。fgets は
     * 終端 NUL に 1 バイト使うため 8300 バイト以上の容量が要る。8192 バイトでは
     * 上限近くの設定が fgets で分断され、後半が黙って失われる。 */
    char line[16384];
    BOOL firstLine = TRUE;
