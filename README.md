...
`--save` でウィンドウの位置・表示状態（通常・最小化・最大化）と Z オーダー（重なり順）を exe と同じディレクトリの `winfocus_positions.dat` に保存する。
最終マウス/キーボード操作から一定時間（デフォルト 10 分）以上経過している場合は保存をスキップする（タスクスケジューラでの定期実行時にユーザ不在の状態で不要な上書きを防ぐ）。
`--restore` で保存した配置に復元し、保存ファイルを削除する。Z オーダーはベストエフォートで再現する（OS の制約により完全保証はできない）。
...
タスクスケジューラから実行する場合、`winfocus.exe` を直接指定せず付属の VBS ラッパーを使用する（`/SUBSYSTEM:WINDOWS` アプリは Task Scheduler が終了を正常に検知できない場合があるため）。

| VBS ファイル | 実行内容 |
|---|---|
| `winfocus-save.vbs` | `--save`（配置保存） |
| `winfocus-restore.vbs` | `--restore`（配置復元） |
...
`--save` のアイドルタイムアウト（分単位）。最終マウス/キーボード操作からこの時間以上経過している場合は保存をスキップする。デフォルト 10 分。
