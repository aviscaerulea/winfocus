...
- すべての可視ウィンドウの最小化・最大化・F11 全画面を解除して通常サイズに復元
...
| `winfocus --restore` | 保存した配置に全ウィンドウを復元する（正常な復元では保存ファイルを削除しない） |
...
- 保存失敗時の挙動：`--save` および引数なし実行で書き込みまたはクローズに失敗した場合、作りかけの不完全な保存ファイルを削除する
- 対象ウィンドウ不在時の挙動：保存対象のウィンドウが 1 つもない場合、前回保存の残存ファイルを削除し、古いデータが残らないようにする
